...
// Sign in a user and get their user ID
...
/**
 * Saves a movie to Firebase Firestore under the logged-in user's watchlist.
 * @param {MovieDetails} movie - The movie object to save.
...
async function saveMovieToFirebase(movie) {
    ...
    if (user) {
        ...
        const userWatchlistRef = firebase.firestore().collection('users').doc(userId).collection('watchlist');

        try {
            await userWatchlistRef.add({
                id: movie.id,
                title: movie.title,
